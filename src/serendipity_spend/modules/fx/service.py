...
def upsert_fx_rate(
    ...
    *,
    ...
    from_currency_norm = normalize_currency(from_currency)
    to_currency_norm = normalize_currency(to_currency)
    if not from_currency_norm:
        raise ValueError("Invalid from_currency")
    if not to_currency_norm:
        raise ValueError("Invalid to_currency")

    fx = session.scalar(
        select(FxRate).where(
            FxRate.claim_id == claim_id,
            FxRate.from_currency == from_currency_norm,
            FxRate.to_currency == to_currency_norm,
        )
    )
    if not fx:
        fx = FxRate(
            claim_id=claim_id,
            from_currency=from_currency_norm,
            to_currency=to_currency_norm,
            rate=rate,
...
def auto_upsert_fx_rates(
    ...
    *,
    ...
    for raw in sorted({c.strip().upper() for c in from_currencies if c}):
        from_currency_norm = normalize_currency(raw)
        if not from_currency_norm:
            skipped.append(raw)
            continue
        if from_currency_norm == to_currency_norm:
            continue
        rate, as_of_date = _fetch_frankfurter_rate(
            from_currency=from_currency_norm, to_currency=to_currency_norm
        )
        fx = upsert_fx_rate(
            session,
            claim_id=claim_id,
...
def _fetch_frankfurter_rate(*, from_currency: str, to_currency: str) -> tuple[Decimal, date]:
    ...
    resp = httpx.get(
        url,
        params={"from": from_currency_norm, "to": to_currency_norm},
        timeout=10,
    ...
    raw_rate = (data.get("rates") or {}).get(to_currency_norm)
